...
export default function Input() {
  ...
  console.log('current user in site---->',session?.user?.image);
    ...
    {session && (
        <div className="flex border-b border-gray-200 p-3 space-x-3">
            ...
            <div className="w-full divide-y divide-gray-200">
                <div className="">
                    <textarea className="w-full border-none focus:ring-0 text-lg placeholderbg-gray-700 tracking-wide min-h-[50px] text-gray-700" rows="2" placeholder="What`s happening"
                    value={input}
                ...
                <div className="flex items-center justify-between pt-2.5">
                    {!loading && (
                        <>
                            ...
                            <button onClick={sendPost} disabled={!input.trim()} className="bg-blue-400 text-white px-4 py-1.5 rounded-full font-bold shadow-md hover:brightness-95 disabled:opacity-50" type="submit">Tweet</button>
                        </>
                    )}
                </div>
            </div>
